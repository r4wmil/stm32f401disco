...
void blink() {
	...
	bool lit = (GPIOD->ODR >> LEDS) & 1U;
	if (lit != (led_count < led_max_count / 2))
		GPIOD->ODR ^= (1U << LEDS);
}
...
void fill_audio_buffer(int16_t *buf, uint32_t count) {
	for (uint32_t i = 0; i < count; i++) {
		buf[i] = sine_table[phase];
...
void DMA1_Stream5_IRQHandler() {
	if (DMA1->HISR & DMA_HISR_HTIF5) {
		...
		fill_audio_buffer(&audio_buf[0], AUDIO_BUF / 2);
	}

	if (DMA1->HISR & DMA_HISR_TCIF5) {
		DMA1->HIFCR = DMA_HIFCR_CTCIF5;
		fill_audio_buffer(&audio_buf[AUDIO_BUF / 2], AUDIO_BUF / 2);
	}
}
